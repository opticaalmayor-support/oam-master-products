  ...
  template: `
    <section class="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div class="flex flex-col gap-3 border-b border-gray-100 p-4 dark:border-gray-700 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 class="text-base font-semibold text-gray-900 dark:text-white">Raw snapshot items</h2>
          <p class="text-xs text-gray-500 dark:text-gray-400">Trace count: {{ traceCount }}</p>
          @if (traceCount > 0) {
            <p class="text-xs text-emerald-600 dark:text-emerald-300">Snapshot items in this run</p>
          }
          @if (insertedCount === 0 && traceCount > 0) {
            <p class="text-xs text-amber-600 dark:text-amber-300">Items were deduplicated in canonical raw storage, but this run snapshot is available.</p>
          }
        </div>
      ...
      @if (isLoading && products.length === 0) {
        ...
        <div class="p-10 text-center">
          <p class="text-sm text-gray-500 dark:text-gray-400">This run has no raw snapshot items.</p>
        </div>
      } @else {
          ...
          <table class="min-w-full text-left text-xs text-gray-500 dark:text-gray-400 md:text-sm">
            <thead class="bg-gray-50 text-[11px] uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-300">
              <tr>
                ...
                <th class="px-3 py-3">Trace type</th>
                <th class="px-3 py-3">Raw ID</th>
                <th class="px-3 py-3">SKU</th>
                <th class="px-3 py-3">Supplier product ID</th>
            ...
            <tbody>
              @for (product of products; track product.id) {
                <tr class="border-t border-gray-100 align-top dark:border-gray-700">
                  <td class="px-3 py-3">
                    @if (getPreviewUrl(product); as previewUrl) {
                      <button type="button" (click)="openGallery(product)" class="block">
                        <img [src]="previewUrl" [alt]="product.name_raw ?? 'preview'" class="h-12 w-12 rounded border border-gray-200 object-cover dark:border-gray-600" />
                      </button>
                    } @else {
                  ...
                  <td class="px-3 py-3">
                    @if (product.raw_id === null) {
                      <span class="rounded bg-amber-100 px-2 py-0.5 text-[11px] font-medium text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">snapshot-only</span>
                    } @else {
                      <span class="rounded bg-emerald-100 px-2 py-0.5 text-[11px] font-medium text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">linked raw</span>
                    }
                  </td>
                  <td class="px-3 py-3">{{ product.raw_id ?? '—' }}</td>
                  <td class="px-3 py-3">{{ product.supplier_sku ?? '—' }}</td>
                  <td class="px-3 py-3">{{ product.supplier_product_id ?? '—' }}</td>
...
export class RawProductsTableComponent implements OnChanges {
  ...
  getPreviewUrl(product: RawProduct): string | null {
    return product.media.thumbnail_main_url ?? product.media.source_urls[0] ?? product.media.full_urls[0] ?? null;
  }
}
